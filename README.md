...
> **MSc Biomolecular Chemistry — UniCT**
> Force field: a99SB-disp + TIP4P-D | Software: GROMACS, PyEMMA, MDAnalysis, SESCA-2025
...
This approach mirrors the ensemble-first strategy validated on α-helical model peptides (CB02, CB03, VW19) and extends it to the disordered/aggregating regime.
...
**Critical**: all systems must have **identical atom counts and ordering**. `00_preprocess.py` will flag any mismatch.
...
Suitable references for Aβ(1-42): Barrow & Zagorski (1991); Jarvet et al. (2007).
Suitable references for Tau(272-314): Mukrasch et al. (2009); von Bergen et al. (2001).
...
> ⚠️ If atom counts differ, re-process topologies in GROMACS (`gmx trjconv -pbc mol`) to a common atom set.
...
Inspect `analysis/plots/its_tica.png` and set `LAG_TICA_FINAL` to the smallest lagtime where the top ITS plateau. Typical values:

| System | Lagtime range |
|---|---|
| Aβ(1-42) at 100 ps/frame | 50–200 frames (5–20 ns) |
...
**Cosine content > 0.5**
→ Simulations not converged. Report as limitation; consider REST2 or replica-exchange.
